...
namespace BNSApp.Solver
{
    /// <summary>
    /// 履歴情報を多く利用するBDMM
    /// ついでにモーションによってコスト計算時に利用するボーンの数を調整する
    /// </summary>
    public class SBDMM : ISolver
    {
            ...
            out Pose nearestCurrentPose, out Pose nearestFuturePose)
        {
            ...
            foreach (var trainMotion in trainData.MotionList)
            {
                ...
                for (var i = pastPoseList.Count; i < trainMotion.Length - pastPoseList.Count; i++)
                {
                    ...
                    if (IsWalkingMotion(testMotionType))
                    {
                        cost = Cost.CalcPartPoseCost(currentTrainPose, currentPose, SkeltonData.GetLowerBoneIndexList());
                    }
                    ...
                    for (var k = 0; k < pastPoseList.Count; k++)
                    {
                        ...
                        if (IsWalkingMotion(testMotionType))
                        {
                            pastCost = Cost.CalcPartPoseCost(pastTrainPose, pastPoseList[k], SkeltonData.GetLowerBoneIndexList());
                        }
